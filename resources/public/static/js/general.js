...
function isValidateDate(dateval,type) {
	if(isValidatePattern(dateval,"DATE")==false)
		return false;
	dateval = dateval.replace(/^\s+/g, '').replace(/\s+$/g, '') 
...
function isValidateDateTime(datetimeval,type) {
	var dateval;
	var timeval;
    if (datetimeval.indexOf(" ")>=0) {
	    dateval = datetimeval.substring(0,datetimeval.indexOf(" "));
		timeval = datetimeval.substring(datetimeval.indexOf(" ")+1,datetimeval.length);
	} else {
		dateval = datetimeval;
	}
	if (isValidatePattern(dateval,"DATE")==false) {
	...
	if(timeval != "") {
		if (isValidatePattern(timeval,"TIME")==false) {
			return false
		}
			
		timeval=timeval.replace(/^\s+/g, '').replace(/\s+$/g, '')
		var hourval=parseInt(timeval.substring(0,timeval.indexOf(":")))
		var minval=parseInt(timeval.substring(timeval.indexOf(":")+1,timeval.length))
...
function isValidatePattern(fldValue,type) {
	if (type.toUpperCase()=="EMAIL") {
		...
		var re = /^\d{4}(\-|\/|\.)\d{1,2}\1\d{1,2} d{1,2}\:\d{1,2}$/
	}
				...
				case "D"  :
					...
					{	
						if(typeof(type[2])=="undefined") var currdatechk="OTH"
						    ...
						    var dtFlag = isValidateDateTime(getObj(fieldname[i]).value,currdatechk);
							var dFlag = isValidateDate(getObj(fieldname[i]).value,currdatechk);
							//alert("date:"+dFlag);
							
							if (!dtFlag && !dFlag) {
							    alert(alert_arr.ENTER_VALID+fieldlabel[i]);
								getObj(fieldname[i]).focus();
								return false;
							}
									if (type[3]) {
...
function previewAttachments(attachementid) {
    var url = "PopupPreviewfile.php?attachementid="+attachementid;
    window.open(url,"PreviewAttachment","menubar=no,toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes")
...
function confirmchedan(url)
{
		if(confirm(alert_arr.SURE_TO_CANCEL_SHOUFEI))
		{
				window.location.href=url;
		}
}
...
function openUITenPopup(module) 
{
	...
	if(module == "Vcontacts") {
		...
		if(flag) 
		{
		    ...
		    var url = "index.php?module="+ module +"&action=Popup&popuptype=specific";  
			for(var i=0;i<document.EditView.elements.length;i++) {
				if(document.EditView.elements[i].name == "campuid") {
					flag = true;
				}
			}
			if(flag) 
			{
				if(document.EditView.campuid.value == '') 
				{
					alert(alert_arr.LBL_PLEASE_SELECT_CAMPU);
					return false;
				}
				url = url + "&campuid=" + document.EditView.campuid.value;
			}
			if(document.EditView.module.value=='Banjikebiaos'){
				var duedate = document.EditView.duedate.value;
				if(duedate == ''){
					alert(alert_arr.LBL_PLEASE_INSERT_DUEDATE);
					document.EditView.duedate.focus();
					return false;
				}

				var timeslot_starthour = document.EditView.timeslot_starthour.value;
				var timeslot_endhour = document.EditView.timeslot_endhour.value;
				timeslot_starthour = Number(timeslot_starthour);
				timeslot_endhour = Number(timeslot_endhour);

				var timeslot_startmin = document.EditView.timeslot_startmin.value;
				var timeslot_endmin = document.EditView.timeslot_endmin.value;
				timeslot_startmin = Number(timeslot_startmin);
				timeslot_endmin = Number(timeslot_endmin);


				if(timeslot_endhour<timeslot_starthour){
					alert('结束时间必须大于开始时间');
					document.EditView.timeslot_endhour.focus();
					return false;
				}else if(timeslot_endhour==timeslot_starthour){
					if(timeslot_endmin==timeslot_startmin ){
						alert('开始时间、结束时间不能相同');
						document.EditView.timeslot_starthour.focus();
						return false;
					}
					if(timeslot_endmin<timeslot_startmin ){
						alert('结束时间必须大于开始时间');
						document.EditView.timeslot_endmin.focus();
						return false;
					}
				}
				url = "index.php?module="+ module +"&action=PopupGetAvailable&popuptype=specific&duedate="+duedate+"&timeslot="+document.EditView.timeslot_starthour.value+':'+document.EditView.timeslot_startmin.value+'-'+document.EditView.timeslot_endhour.value+':'+document.EditView.timeslot_endmin.value; 
			}else if(document.EditView.module.value=='Banjikaoqins'){
				var duedate = document.EditView.duedate.value;
				if(duedate == ''){
					alert(alert_arr.LBL_PLEASE_INSERT_DUEDATE);
					document.EditView.duedate.focus();
				...
				url = "index.php?module="+ module +"&action=PopupGetAvailable&popuptype=specific&duedate="+duedate+"&timeslot="+document.EditView.timeslot.value; 
			}

			return window.open(url,"test","width=1020,height=602,resizable=1,scrollbars=0");
	}else if (module == "Banjis") {
			for(var i=0;i<document.EditView.elements.length;i++) {
				if(document.EditView.elements[i].name == "campuid") {
					flag = true;
				}
	...
	}else if (module == "Classrooms") {
		   if(document.EditView.module.value=='Classnos'){
		   	    var url = "index.php?module="+ module +"&action=Popup&popuptype=specific_model_notnormal&columnname=classroomname";
		   }else{
		   	    var url = "index.php?module="+ module +"&action=Popup&popuptype=specific";
		   }
			for(var i=0;i<document.EditView.elements.length;i++) {
			...
			return window.open(url,"test","width=1120,height=602,resizable=1,scrollbars=0");
	}else if (module == "Classnos") {
			var url = "index.php?module="+ module +"&action=Popup&popuptype=specific";
			for(var i=0;i<document.EditView.elements.length;i++) {
				if(document.EditView.elements[i].name == "classroomsid") {
					flag = true;
				}
			}
			if(flag) 
			{
				if(document.EditView.classroomsid.value == '') 
				{
					alert(alert_arr.LBL_PLEASE_SELECT_CLASSROOM);
					return false;
				}
				url = url + "&classroomsid=" + document.EditView.classroomsid.value;
			}
			return window.open(url,"test","width=1020,height=602,resizable=1,scrollbars=0");
	}else if (module == "Courses") {
			for(var i=0;i<document.EditView.elements.length;i++) {
				if(document.EditView.elements[i].name == "banjisid") {
					flag = true;
				}
			}
			if(flag) 
			{
				if(document.EditView.banjisid.value == '') 
				{
					alert(alert_arr.LBL_PLEASE_SELECT_BANJI_FIRST);
					return false;
				}
				url = url + "&banjisid=" + document.EditView.banjisid.value;
			}
	}
...
function setkeshi(){
	...
	if(chargetype == "计次收费"){
		document.getElementById("keshi").readOnly = "readonly";
	}
	else{
		document.getElementById("keshi").removeAttribute("readonly");
	}
...
function showRelateMonthKeCheng(relatemodule,record,popuptype){
	var url='&record='+record+'&popuptype='+popuptype+'&relatemodule='+relatemodule;
	getMonthScheduList_js(relatemodule,url);
}


function getMonthScheduList_js(module,url){
	Jquery("#status").css("display","block");
	if(('show_search_url').value!='')
		var urlstring = $('show_search_url').value;
	else
		var urlstring = '';
	Jquery("#status").css("display","inline");
		Jquery.ajax({
	    url: "index.php",
		data: "module=Users&action=UsersAjax&file=showRelatedModuleMonthSchedu"+urlstring+"&"+url,
		success:function(rest){//alert(rest);return;
		    Jquery("#status").css("display","none");
		    Jquery("#ListViewSchedu").html(rest);	
		}
	});
}
...
function getRelatePopupKeChengList(url){
	Jquery.ajax({
		url:"index.php",
		data:'module=Users&action=UsersAjax&file=setRelateBingKeCheng'+url,
		success:function(rest){
			if(rest != ""){
				Jquery("#ListViewSchedu").html(rest);
			}
...
function RelateBingKeCheng(relatemodule,record,popuptype){
	var url='&record='+record+'&popuptype='+popuptype+'&relatemodule='+relatemodule;
	getRelatePopupKeChengList(url);
}
function showRelateDefaultWeek(relatemodule,record,popuptype){
	var sltYear=$("sltYear").value;
	var sltWeek=$("sltWeek").value;
	var url='&record='+record+'&popuptype='+popuptype+'&relatemodule='+relatemodule+'&sltYear='+sltYear+'&sltWeek='+sltWeek;
	getRelatePopupKeChengList(url);
	
}

function callRelateKeChengSeach(relatemodule,record,popuptype){
	var sltYear=$("sltYear").value;
	var sltWeek=$("sltWeek").value;
	var url='&record='+record+'&popuptype='+popuptype+'&relatemodule='+relatemodule+'&sltYear='+sltYear+'&sltWeek='+sltWeek;
	getRelatePopupKeChengList(url);
...
function showKeChengLockDiv(schid){	
	if(Jquery("#kechenglist_"+schid).css("display") == 'none'){
		show("kechenglist_"+schid);
		var imghtml='<a href="javascript:;" onclick="showKeChengLockDiv('+schid+');"><img border="0"  src="themes/softed/images/minus.gif"></a>';
	}else{
		Jquery("#kechenglist_"+schid).css("display",'none');
		var imghtml='<a href="javascript:;" onclick="showKeChengLockDiv('+schid+');"><img border="0"  src="themes/softed/images/plus.gif"></a>';
	}
	Jquery("#showimg_"+schid).html(imghtml);
}


function show_yearmonthschedu(module,mark,symbol,month){
	var thisdate=$("fdstartdate").value;
	thisdate=thisdate.split("-");
	var stdthisyear=thisdate[0];
	var stdthismonth=thisdate[1];
	var stdthisday=thisdate[2];
	if(month!=''){
		var fdstartdate=stdthisyear+'-'+month+'-'+stdthisday;
		var url='backshowmg=false&fdstartdate='+fdstartdate;
	}else{
		stdthisyear=Number(stdthisyear);
		stdthismonth=Number(stdthismonth);
		if(mark=='year'){
			if(symbol=='Inc'){
				stdthisyear++;
			}else if(symbol=='Dec'){
			...
			if(stdthismonth<10){
				stdthismonth="0"+stdthismonth;
			}
		}
		var fdstartdate=stdthisyear+'-'+stdthismonth+'-'+stdthisday;
		var url='backshowmg=false&fdstartdate='+fdstartdate;
	}
...
* 列表显示一级分类
*/
function getWarehouseView(module,file,showmode){
	var schview='';
	if(arguments[3]!=''){
		schview=arguments[3];//二级分类显示
	}
	$("status").style.display='inline';
	if($('search_url').value!='')
	...
	new Ajax.Request(
		...
		{queue: {position: 'end', scope: 'command'},
		method: 'post',
		postBody:"module="+module+"&action="+module+"Ajax&file="+file+"&ajax=true"+urlstring+"&showmode="+showmode+"&schview="+schview,
		onComplete: function(response) {
				$("status").style.display="none";
...
function showKechengInsertInfo(module,accountid,crmid,shoufeiid){
	$("status").style.display="block";
	new Ajax.Request(
		'index.php',
		{queue: {position: 'end', scope: 'command'},
		method: 'post',
		postBody:"module="+module+"&action="+module+"Ajax&file=ValidataPaiKe&ajax=true&accountid="+accountid+"&crmid="+crmid+"&return_module=Acctolists&return_action=DetailView&return_id="+crmid+"&shoufeiid="+shoufeiid,
		onComplete: function(response) {
			$("status").style.display="none";
				result = response.responseText;
				$("crmtableserialinfodiv").innerHTML= result;
				calcReleInfo();
			}
		}
	);
...
* div 显示 月度课表
*/
function showMonthScheduDiv(relate_module,record){
	if($('search_url').value!='')
	...
	Jquery("#status").css("display","inline");
		Jquery.ajax({
	    url: "index.php",
		data: "module=Banjis&action=BanjisAjax&file=showMonthSchedu"+urlstring+'&relate_module='+relate_module+'&banjisid='+record,
		success:function(rest){//alert(rest);return;
		    Jquery("#status").css("display","none");
		    Jquery("#show-related-info-disp").html(rest);
...
/*
* 相关信息 关联选择其他模块数据时可以选择某个视图的数据
*/
function loadRelatedModname_click(module,related_modname,entityid,type){
	var selectObj=document.getElementById("clucvid");
	var cvid=selectObj.options[selectObj.selectedIndex].value;
	if(type!='delete'){
		if(cvid==''){alert('请选择视图');return ;}
	}
	if(type=='delete' || type=='del'){
		if(!confirm(alert_arr.SURE_TO_DELETE))return ;
	}
	Jquery("#status").css("display","inline");
	Jquery.ajax({
		url: "index.php",
		data: "module=Users&action=UsersAjax&file=LoadRelatedModule&entityid="+entityid+"&cvid="+cvid+"&related_modname="+related_modname+"&modname="+module+"&type="+type,
		success:function(rest){
			Jquery("#status").css("display","none");
			Jquery("#issubmit").val(rest);
...
function getListViewImgData(module){
	if(getObj('selected_id')==undefined){
       return ;
	}
	var relObj = getObj('selected_id');
	var idstring = '';
	for(var i = 0 ; i<relObj.length ;i++){
		idstring +=relObj[i].value+';';
	}
	Jquery("#status").css("display","inline");
		Jquery.ajax({
		url: "index.php",
		data: "module=Users&action=UsersAjax&file=setRelatedListViewImg&related_modname="+module+"&idstring="+idstring,
		success:function(rest){//alert(rest);//return;
			Jquery("#status").css("display","none");
			if(rest != ''){
				var relatedobj = eval('('+ rest +')')?eval('('+ rest +')'):JSON.parse(rest);
				Jquery.each(relatedobj,function(entity_id,imghtml){
					  Jquery("#showimguitype_"+entity_id).html(imghtml);
				   } 
				);
			}
		}
...
*点击 打开新标签页面
*/
function callNewPage(url){
	open(url,"");
}


function showRelatedListViewImageDiv(obj,divid,name){
	var tagName=document.getElementById(divid);

	Jquery("#showimagetitle").html(name);
	if(typeof(tagName) == 'undefined') {
		return;
	}
    var leftSide = findPosX(obj);
...
/*
*查看更多图片处理，列表页面/详细页面均可使用 add by lihaiping 2014-05-23
*/
function setRelatedProdImages(module,entity_id,record){
	var fromaction = '';
	if(arguments[3]!=undefined)fromaction=arguments[3];
    Jquery("#status").css("display","inline");
    Jquery.ajax({
    	url:"index.php",
    	data:"module=Users&action=UsersAjax&file=getRelatedModnameImgList&ajax=true&entity_id="+entity_id+"&related_modname="+module+"&record="+record+"&fromaction="+fromaction,
    	success:function(rest){//alert(rest);
    		 Jquery("#status").css("display","none");
    		 if(rest != ''){
				var relatedobj = eval('('+ rest +')')?eval('('+ rest +')'):JSON.parse(rest);
				Jquery.each(relatedobj,function(productsid,imghtml){
					 Jquery("#showimageserialinfodiv").html(imghtml.html);
					 Jquery("#showcurrentcountimg").html(imghtml.noofrows);
					 Jquery("#showturnImg_left").html(imghtml.last);
					 Jquery("#showturnImg_right").html(imghtml.next);
					 Jquery("#trshowimgage_"+module).css("display","");
				   } 
				);
			}
    	}
...
*显示图片logo
*/
function setRelatedProdImagesLogo(module,entity_id){
    Jquery("#status").css("display","inline");
    Jquery.ajax({
    	url:"index.php",
    	data:"module=Users&action=UsersAjax&file=getRelatedModnameImgLogo&ajax=true&entity_id="+entity_id+"&related_modname="+module,
    	success:function(rest){//alert(rest);
    		 Jquery("#status").css("display","none");
    		 if(rest != ''){
				Jquery("#showimagelogo_"+entity_id).html(rest);
				Jquery("#showimagelogo_"+entity_id).css("display","block");
			}
    	}
    });
}
...
function OnMouseOvershowDetailViewSmallImage(obj,related_modname,image_id,divid){
	var tagName=document.getElementById(divid);
	if(typeof(tagName) == 'undefined') {
		return;
	}
    ...
    if(getVal  > document.body.clientWidth ){
        ...
        tagName.style.left= leftSide + 'px';
    tagName.style.top= topSide +18 + 'px';

    tagName.style.display='';

    Jquery.ajax({
    	url:"index.php",
    	data:"module=Users&action=UsersAjax&file=setRelatedDetailViewSmallImage&image_id="+image_id+"&related_modname="+related_modname,
    	success:function(rest){
    		//alert(rest)
    		Jquery("#smallimageserialinfodiv").html(rest);
    	}
    });
}


function OnMouseOutshowDetailViewSmallImage(divid){

	var div = document.getElementById("showdetailviewsmallimagediv");  
    var x=event.clientX;  
    var y=event.clientY;  
    ...
    if( x < divx1 || x > divx2 || y < divy1 || y > divy2){  
		...
		Jquery("#smallimageserialinfodiv").html('');
		tagName.style.display='none';
    }  	
}